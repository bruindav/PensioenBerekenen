...
// v8: parser volledig herschreven — blokken zijn periodes, polis startleeftijd =
//     vroegste blok waarin polis verschijnt, totLeeftijd = altijd null (levenslang)
//     tenzij polis verdwijnt in later blok. Tijdlijn begint bij vroegste uitkering.
...
// ─── MPO JSON parser ──────────────────────────────────────────────────────────
// Blokken beschrijven de situatie per periode. Per polis:
//   startLeeftijd = vroegste blok waarin polis verschijnt
//   totLeeftijd   = null (levenslang) tenzij polis verdwijnt in later blok
//   bedrag        = uit het blok met hoogste startleeftijd (meest actueel)
function parseerJSON(tekst) {
  const data    = JSON.parse(tekst);
  const details = data?.Details?.OuderdomsPensioenDetails?.OuderdomsPensioen ?? [];

  const polisFirstSeen = {}; // herkenning -> { startLft, naam, standPer }
  const polisLastSeen  = {}; // herkenning -> { bedrag, isLevenslang }

  details.forEach((blok) => {
    const startLft     = (blok.Van?.Leeftijd?.Jaren ?? 67) + (blok.Van?.Leeftijd?.Maanden ?? 0) / 12;
    const isLevenslang = !!blok.Tot?.OuderdomsPensioenEvent; // "Overlijden"

    (blok.IndicatiefPensioen ?? []).forEach((p) => {
      const h = p.HerkenningsNummer; if (!h) return;
      const bedrag = p.Opgebouwd ?? p.TeBereiken ?? 0;
      // Eerste keer gezien → bewaar vroegste startleeftijd
      if (!polisFirstSeen[h]) {
        polisFirstSeen[h] = { startLft, naam: p.PensioenUitvoerder, standPer: p.StandPer };
      }
      // Altijd updaten met meest recente bedrag en levenslang-status
      polisLastSeen[h] = { bedrag, isLevenslang };
    });
  });

  // Detecteer polissen die verdwijnen (stopt voor overlijden)
  // Een polis stopt als hij NIET in het laatste blok (levenslang) voorkomt
  const polissen = Object.keys(polisFirstSeen).map(h => ({
    id:           `${h}@${polisFirstSeen[h].startLft}`,
    naam:         polisFirstSeen[h].naam,
    herkenning:   h,
    type:         "pensioen",
    bruto_jaar:   polisLastSeen[h].bedrag,
    startLeeftijd: polisFirstSeen[h].startLft,
    totLeeftijd:  polisLastSeen[h].isLevenslang ? null : (() => {
      // Zoek het blok NADAT deze polis voor het last verscheen
      // Niet-levenslang = stopt op de totLeeftijd van het laatste blok waarin hij voorkwam
      const laastBlokMetPolis = [...details].reverse().find(blok =>
        (blok.IndicatiefPensioen ?? []).some(p => p.HerkenningsNummer === h)
      );
      const tot = laastBlokMetPolis?.Tot?.Leeftijd;
      return tot ? tot.Jaren + (tot.Maanden ?? 0) / 12 : null;
    })(),
    standPer:     polisFirstSeen[h].standPer,
  }));

  // AOW: neem het blok met het hoogste OpgebouwdSamenwonend
...
// ─── MPO XML parser ───────────────────────────────────────────────────────────
// Zelfde principe: vroegste verschijning = startLeeftijd, levenslang tenzij verdwijnt
function parseerXML(tekst) {
  ...
  for (let b = 0; b < blokken.length; b++) {
    ...
    const startLft = (parseInt(vanEl ? t(vanEl, "Jaren") || "67" : "67")) +
                     (parseInt(vanEl ? t(vanEl, "Maanden") || "0" : "0")) / 12;

    // Levenslang = Tot heeft <Overlijden/> of geen Leeftijd
    const totEl    = g(blok, "Tot")[0];
    const isLevenslang = totEl && t(totEl, "Jaren") === "";
    const totLftNum = totEl && t(totEl, "Jaren") !== ""
      ? parseInt(t(totEl, "Jaren") || "0") + parseInt(t(totEl, "Maanden") || "0") / 12
      : null;

    const aowEl = g(blok, "AOWDetailsOpbouw")[0];
    if (aowEl) {
      const s = parseInt(t(aowEl, "OpgebouwdSamenwonend") || "0");
      if (s > aowSamen) { aowSamen = s; aowAlleen = parseInt(t(aowEl, "OpgebouwdAlleenstaand") || "0"); aowStart = startLft; }
    }
    ...
    for (let p = 0; p < polissen.length; p++) {
      ...
      if (!polisFirstSeen[h]) {
        polisFirstSeen[h] = { startLft, naam: t(pEl, "PensioenUitvoerder") || "Onbekend", standPer: t(pEl, "StandPer") };
      }
...
export default function PensioenApp() {
  ...
  // ─── Sortering: oudste (laagste geboortejaar) eerst ──────────────────────────
  const personenGesorteerd = useMemo(() =>
    [...personen].sort((a, b) => a.geboortejaar - b.geboortejaar), [personen]);

  // ─── Startjaar: jaar van vroegste uitkering van de oudste persoon ─────────────
  const startJaar = useMemo(() => {
    ...
    const vroegsteLft  = eigenPens.length > 0
      ? Math.min(...eigenPens.map(p => p.startLeeftijd))
      : oudste.pensioenLeeftijd;
    return oudste.geboortejaar + Math.floor(vroegsteLft);
  }, [personenGesorteerd, pensioenen]);

  // ─── Pensioenmomenten (voor referentielijnen) ─────────────────────────────────
  const pensioenmomenten = useMemo(() =>
    personenGesorteerd.map((p, pi) => ({
      jaar:  p.geboortejaar + Math.floor(p.pensioenLeeftijd),
      naam:  p.naam.split(" ")[0],
  ...
  const chartData = useMemo(() => {
    ...
    return Array.from({ length: 30 }, (_, i) => {
      ...
      personen.forEach((persoon) => {
        ...
        // Pensioen: actief als lft >= startLeeftijd EN (totLeeftijd null OF lft < totLeeftijd)
        let pensioenBruto = 0;
        pensioenen.filter(p => p.eigenaarId === persoon.id).forEach((p) => {
          const gestart = lft >= p.startLeeftijd;
          const gestopt = p.totLeeftijd !== null && p.totLeeftijd !== undefined && lft >= p.totLeeftijd;
          if (gestart && !gestopt) {
            if (p.type === "bankspaar") {
        ...
        if (persoon.id === personenGesorteerd[0]?.id && simulatie.aankoopJaar > 0) {
          ...
          if (jaar >= simStart) pensioenBruto += simulatie.aankoopUitkering * 12;
        }
        ...
        pp[persoon.id] = { lft: lft.toFixed(1), pensioenBruto: Math.round(pensioenBruto), aowBruto, heeftAow };
      ...
      const rij = {
        jaar,
        pensioenBruto: Math.round(totPensioen), aowBruto: Math.round(totAow),
        spaargeld: Math.round(spaargeld), woning: Math.round(woning),
        totalBruto, totalNetto: Math.round(totNetto),
        totalNettoMaand: Math.round(totNetto / 12), totalBrutoMaand: Math.round(totalBruto / 12),
      };
    ...
    <div style={{ fontFamily: "'Georgia',serif", background: "#0f1923", minHeight: "100vh", color: "#e8dcc8" }}>
      ...
      <div style={{ maxWidth: 980, margin: "0 auto", padding: "28px 20px" }}>
            ...
            <div key={persoon.id} style={{ background: "#1a2d3d", border: `1px solid ${KLEUREN[pi % KLEUREN.length]}44`, borderRadius: 12, padding: 18, marginBottom: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                <h3 style={{ margin: 0, color: KLEUREN[pi % KLEUREN.length], fontSize: 15 }}>
                  {pi === 0 ? "👤 " : "👥 "}{persoon.naam}
                  {pi === 0 && <span style={{ fontSize: 11, color: "#555", marginLeft: 8 }}>· oudste</span>}
                </h3>
            ...
            <div style={{ padding: 20, background: "#1a2d3d", borderRadius: 12, border: "1px solid #2a4a5e" }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
                {pensioenmomenten.map((m, i) => {
                  ...
                  const persoon = personenGesorteerd[i];
                  return (
                    <div key={i} style={{ background: "#111d26", border: `1px solid ${m.kleur}44`, borderRadius: 10, padding: "12px 16px", minWidth: 180 }}>
                      <div style={{ color: m.kleur, fontSize: 12, fontWeight: 600, marginBottom: 2 }}>📍 {persoon.naam}</div>
                      <div style={{ color: "#7a9bb0", fontSize: 11, marginBottom: 8 }}>{m.jaar} · leeftijd {persoon.pensioenLeeftijd}</div>
                      {rij && <>
                        <div style={{ color: "#c9a84c", fontSize: 12 }}>€ {rij.totalBrutoMaand.toLocaleString("nl-NL")}/mnd bruto</div>
                        <div style={{ color: "#4caf8a", fontSize: 16, fontWeight: 700 }}>€ {rij.totalNettoMaand.toLocaleString("nl-NL")}/mnd netto</div>
                      </>}
                    </div>
                  );
                })}
        ...
        {tab === "pensioenen" && <Section title="Pensioenen & producten">
          ...
          <button onClick={() => { const id = personenGesorteerd[0]?.id ?? "onbekend"; setPensioenen([...pensioenen, { id: `handmatig_${Date.now()}`, naam: "Nieuw pensioen", type: "pensioen", eigenaarId: id, bruto_jaar: 0, startLeeftijd: 67.25, totLeeftijd: null }]); }}
        ...
        {tab === "simulatie" && <Section title="Pensioen aankoop simulatie">
          <p style={{ color: "#7a9bb0", fontSize: 13, marginBottom: 18 }}>Speel ook met partner-pensioenleeftijd via het Profiel tabblad.</p>
          <Grid>
          ...
          <div style={{ background: "#1a2d3d", borderRadius: 12, padding: 18, marginBottom: 20 }}>
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={chartData}>
                ...
                <Tooltip content={({ active, payload, label }) => {
                  ...
                  return (
                    <div style={{ background: "#0f1923", border: "1px solid #2a4a5e", borderRadius: 8, padding: "10px 14px", fontSize: 12 }}>
                      ...
                      {personenGesorteerd.map((p, pi) => (
                        <div key={p.id} style={{ color: KLEUREN[pi % KLEUREN.length], marginBottom: 2 }}>
                          {p.naam.split(" ")[0]}: {rij?.[`lft_${p.id}`]} jr · € {(rij?.[`pen_${p.id}`] ?? 0).toLocaleString("nl-NL")} pensioen
                        </div>
                      ))}
                ...
                {personenGesorteerd.map((p, pi) => (
                  <Line key={p.id} type="monotone" dataKey={`pen_${p.id}`} name={`Pensioen ${p.naam.split(" ")[0]}`} stroke={KLEUREN[pi % KLEUREN.length]} strokeWidth={1} dot={false} strokeDasharray="6 3" />
                ))}
              </LineChart>
            </ResponsiveContainer>
          </div>
                ...
                {chartData.map((r, i) => {
                  ...
                  return (
                    <tr key={i} style={{ background: isPensioenMoment ? "#1a2d1a" : i % 2 === 0 ? "#111d26" : "#0f1923", borderTop: isPensioenMoment ? "2px solid #4caf8a44" : undefined }}>
                      ...
                      {personenGesorteerd.map((p, pi) => (
                        <td key={p.id} style={{ ...cel, color: parseFloat(r[`lft_${p.id}`]) >= p.pensioenLeeftijd ? KLEUREN[pi % KLEUREN.length] : "#4a6a7e" }}>
                          {r[`lft_${p.id}`]}
                        </td>
                      ))}
  ...
  return (
    <div style={{ background: "#111d26", border: `1px solid ${kleur}33`, borderRadius: 10, padding: 14, marginBottom: 10 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11, color: "#4a6a7e", marginLeft: 24 }}>
            {p.herkenning && `#${p.herkenning} · `}
            start lft {p.startLeeftijd}
            {p.totLeeftijd ? ` · stopt lft ${p.totLeeftijd}` : " · levenslang"}
            {p.standPer ? ` · ${p.standPer}` : ""}
          </div>
        </div>
      ...
      <Grid>
        ...
        <Field label="Stopt op leeftijd (leeg=levenslang)" value={p.totLeeftijd ?? ""} onChange={v => update("totLeeftijd", v === "" ? null : +v)} type="number" />
        {p.type === "bankspaar"
