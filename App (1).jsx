...
function parseerMPO(data) {
  ...
  details.forEach((blok) => {
    ...
    (blok.IndicatiefPensioen ?? []).forEach((p) => {
      ...
      // Unieke sleutel per herkenning+startleeftijd
  ...
  Object.values(resultMap)
    ...
    .forEach((p) => {
      ...
      if (!seen[base]) {
        ...
        // Ander bedrag op hogere leeftijd → aparte regel
        dedup[p.id] = p;
        seen[base] = p.bruto_jaar;
      }
    });
  ...
  return { pensioenen: Object.values(dedup), aow: { opgebouwdSamen: aowOpgebouwdSamen, opgebouwdAlleen: aowOpgebouwdAlleen } };
}
...
export default function PensioenApp() {
  ...
  function importeerMPO(e) {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (file.name.endsWith(".json")) {
          result = parseerMPO(JSON.parse(ev.target.result));
        ...
        if (result.pensioenen.length === 0) {
          setImportStatus({ ok: false, tekst: "❌ Geen pensioenregelingen gevonden in dit bestand." });
          return;
        }

        const nieuwePensioenen = result.pensioenen.map((p, i) => ({ ...p, id: p.id || `import_${Date.now()}_${i}` }));
        setPensioenen(nieuwePensioenen);

        const nieuwProfiel = result.aow.opgebouwdSamen > 0
          ? { ...profiel, aowOpgebouwdSamen: result.aow.opgebouwdSamen, aowOpgebouwdAlleen: result.aow.opgebouwdAlleen }
          : profiel;
        if (result.aow.opgebouwdSamen > 0) setProfiel(nieuwProfiel);

        setImportStatus({
          ok: true,
          tekst: `✅ ${result.pensioenen.length} pensioenregelingen ingeladen${result.aow.opgebouwdSamen > 0 ? ` · AOW opgebouwd € ${result.aow.opgebouwdSamen.toLocaleString("nl-NL")}/jr` : ""}`,
        });
        setTab("pensioenen");
      } catch (err) {
    ...
    reader.readAsText(file);  // ← alleen hier, geen dubbele aanroep
  }
  ...
  return (
    <div style={{ fontFamily: "'Georgia',serif", background: "#0f1923", minHeight: "100vh", color: "#e8dcc8" }}>
      ...
      <div style={{ background: "linear-gradient(135deg,#1a2d3d,#0f1923)", borderBottom: "1px solid #2a4a5e", padding: "16px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: "#c9a84c" }}>🏦 Pensioen Planner</h1>
